"""
Quick WS test for in-meeting flow (final transcript -> intent/recap state).

Usage:
  python vnpt_ai_hackathon/backend/tests/ws_in_meeting_quick.py

Env (optional):
  MEETMATE_HTTP_BASE=http://localhost:8000
  MEETMATE_WS_BASE=ws://localhost:8000
  MEETMATE_SESSION_ID=<uuid>
  MEETMATE_CREATE_SESSION=1
  MEETMATE_MAX_STATES=3
  MEETMATE_SEND_DELAY_SEC=1.0
  MEETMATE_PRINT_JSON=0
"""

import asyncio
import json
import os
import time
import uuid
from typing import Any, Dict
from urllib.error import URLError
from urllib.request import Request, urlopen

try:
    import websockets
except Exception as exc:  # pragma: no cover - manual script
    raise SystemExit("Missing dependency: pip install websockets") from exc


HTTP_BASE = os.getenv("MEETMATE_HTTP_BASE", "https://vnpt-ai-hackathon-meetmate.onrender.com").strip()
if HTTP_BASE and "://" not in HTTP_BASE and not HTTP_BASE.startswith(("ws://", "wss://")):
    HTTP_BASE = "https://" + HTTP_BASE
HTTP_BASE = HTTP_BASE.rstrip("/")
WS_BASE = os.getenv("MEETMATE_WS_BASE", "").strip()
SESSION_ID = os.getenv("MEETMATE_SESSION_ID", "").strip()
CREATE_SESSION = os.getenv("MEETMATE_CREATE_SESSION", "1").lower() not in ("0", "false", "no")
MAX_STATES = int(os.getenv("MEETMATE_MAX_STATES", "3"))
SEND_DELAY_SEC = float(os.getenv("MEETMATE_SEND_DELAY_SEC", "1.0"))
PRINT_JSON = os.getenv("MEETMATE_PRINT_JSON", "0").lower() in ("1", "true", "yes")
OVERALL_TIMEOUT_SEC = float(os.getenv("MEETMATE_OVERALL_TIMEOUT_SEC", "30"))


def _ws_base_from_http(http_base: str) -> str:
    if http_base.startswith("https://"):
        return "wss://" + http_base[len("https://") :]
    if http_base.startswith("http://"):
        return "ws://" + http_base[len("http://") :]
    if http_base.startswith("ws://") or http_base.startswith("wss://"):
        return http_base
    return "ws://" + http_base


def _post_json(url: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    data = json.dumps(payload).encode("utf-8")
    req = Request(url, data=data, headers={"Content-Type": "application/json"}, method="POST")
    try:
        with urlopen(req, timeout=10) as resp:
            body = resp.read().decode("utf-8")
        return json.loads(body)
    except (URLError, OSError) as exc:
        raise SystemExit(
            f"Cannot reach backend at {HTTP_BASE}. Start the server or set MEETMATE_HTTP_BASE. Error: {exc}"
        ) from exc


def _ensure_session() -> str:
    global SESSION_ID
    if SESSION_ID:
        return SESSION_ID
    if not CREATE_SESSION:
        raise SystemExit("Set MEETMATE_SESSION_ID or enable MEETMATE_CREATE_SESSION=1.")
    payload = {
        "session_id": None,
        "language_code": "vi-VN",
        "target_sample_rate_hz": 16000,
        "audio_encoding": "PCM_S16LE",
        "channels": 1,
        "realtime": True,
        "interim_results": True,
        "enable_word_time_offsets": True,
    }
    resp = _post_json(f"{HTTP_BASE}/api/v1/sessions", payload)
    SESSION_ID = resp["session_id"]
    return SESSION_ID


async def _listen_frontend(stop: asyncio.Event) -> None:
    url = f"{WS_BASE}/api/v1/ws/frontend/{SESSION_ID}"
    count = 0
    async with websockets.connect(url, ping_interval=20, ping_timeout=20, close_timeout=5) as ws:
        while not stop.is_set():
            try:
                msg = await asyncio.wait_for(ws.recv(), timeout=1)
            except asyncio.TimeoutError:
                continue
            data = json.loads(msg)
            event = data.get("event")
            if event != "state":
                continue
            payload = data.get("payload") or {}
            print(
                "STATE intent=",
                payload.get("semantic_intent_label"),
                "topic=",
                payload.get("current_topic_id"),
                "recap=",
                payload.get("live_recap"),
            )
            if PRINT_JSON:
                print(json.dumps(payload, ensure_ascii=False))
            count += 1
            if count >= MAX_STATES:
                stop.set()


async def _send_transcripts(stop: asyncio.Event) -> None:
    url = f"{WS_BASE}/api/v1/ws/in-meeting/{SESSION_ID}"
    async with websockets.connect(url, ping_interval=20, ping_timeout=20, close_timeout=5) as ws:
        try:
            hello = await asyncio.wait_for(ws.recv(), timeout=2)
            print("WS connected:", hello)
        except asyncio.TimeoutError:
            pass

        chunks = [
            {
                "meeting_id": SESSION_ID,
                "chunk": "Core Banking tien do 68%. Batch processing dang toi uu.",
                "speaker": "SPEAKER_01",
                "time_start": 0.0,
                "time_end": 12.0,
                "is_final": True,
                "confidence": 0.9,
                "lang": "vi",
            },
            {
                "meeting_id": SESSION_ID,
                "chunk": "Can 2 senior dev de giu timeline 01/01. Security con 3 medium issues.",
                "speaker": "SPEAKER_01",
                "time_start": 12.0,
                "time_end": 72.0,
                "is_final": True,
                "confidence": 0.9,
                "lang": "vi",
            },
        ]

        for payload in chunks:
            await ws.send(json.dumps(payload))
            try:
                ack = await asyncio.wait_for(ws.recv(), timeout=3)
                print("ACK:", ack)
            except asyncio.TimeoutError:
                print("WARN: no ingest_ack")
            await asyncio.sleep(SEND_DELAY_SEC)

        while not stop.is_set():
            await asyncio.sleep(0.2)


async def _stop_after(stop: asyncio.Event, timeout_sec: float) -> None:
    await asyncio.sleep(timeout_sec)
    if not stop.is_set():
        print(f"Timeout after {timeout_sec:.0f}s without enough state events.")
        stop.set()


async def main() -> None:
    global WS_BASE
    session_id = _ensure_session()
    if not WS_BASE:
        WS_BASE = _ws_base_from_http(HTTP_BASE).rstrip("/")
    print("HTTP_BASE:", HTTP_BASE)
    print("WS_BASE:", WS_BASE)
    print("SESSION_ID:", session_id)

    stop = asyncio.Event()
    await asyncio.gather(_listen_frontend(stop), _send_transcripts(stop), _stop_after(stop, OVERALL_TIMEOUT_SEC))


if __name__ == "__main__":
    asyncio.run(main())
